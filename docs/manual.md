...
Генерация специального селектора: который позволяет сделать запрос без условий. Но с указанным лимитом, для защиты от ошибки и невозможности таким образом сделать плохо БД. Максимальный лимит 20000. При достижении ограничения будут выведено предупреждение. Как правило словари не превышают 20 тыс. записей.
...
Условные индексы не поддерживаются.

!!!*Для PostgreSQL*

Для корректной работы планировщика необходимо указывать все условия: которые были указаны при создании индекса.
...
## Хелперы для конфигурирования коробки или получения схемы БД
...
Для всех неуникальных ключей у селектора присутствует дополнительный параметр `limiter` с интерфейсом `activerecord.SelectorLimiter` который ограничивает выборку по неуникальному ключу, и даёт возможность установить `offset`. При селекте по нескольким ключам или по неуникальному полю важно проверять достигли лимита или нет, если это используется для словарей, когда всё надо достать за один поход и важно не пропустить момент, когда лимит достигнут, то необходимо выставить FulfillWarn в true. (!Не реализовано Если селект идёт по ключу в разные шарды то лимит действует на каждый шард! Возвращено может быть limit * shardCount записей!)
...
- временным
  ...
  - `select_process`
  - `select_newobj`
  - `delete_box` - Deprecated!
  - `delete_db`
  - `update_box` - Deprecated!
  - `update_db`
  - `insertreplace_packtuple`
  - `insertreplace_pack`
  - `insertreplace_box` - Deprecated!
  - `insertreplace_db`
  - `call_proc`
- статистическим
  - `insert_success`
...
- ошибочным
  ...
  - `insertreplace_preparebox` - Deprecated!
  - `insertreplace_preparedb`
  - `insertreplace_prespreparebox` - Deprecated!
  - `insertreplace_prespreparedb`
  - `replace_notexists`
  - `select_box` - Deprecated!
  - `select_db`
  - `select_preparebox`
  ...
  - `update_notexists`
  - `update_packpk`
  - `update_preparebox` - Deprecated!
  - `update_preparedb`
  - `update_resp`
  - `call_proc`
  - `call_proc_preparebox` - Deprecated!
  - `call_proc_preparedb`
...
В результате его работы будет сгенерированы файлы:

- `model/repository/cmpl/foo/octopus.go`
- `model/repository/cmpl/bar/postgres.go`

#### Функции

Создание нового пустого объекта - `foo.New` или `bar.New`
...
`cmd/argen/main.go` при наличии ключа fixture_path генерирует в папку по этому пути в пакете `fixture` файлы доступа к фикстурам данных, описываемых в одноименных с именами сущностей файлах, которые должны находится в папке `{fixture_path}/fixture/data`
