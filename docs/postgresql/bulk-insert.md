...
4. **Нет простого BulkInsert (без ON CONFLICT)**
   - PostgreSQL не гарантирует порядок строк в RETURNING — он может
     отличаться от порядка VALUES. Без ON CONFLICT нет уникального ключа
     для сопоставления возвращённых строк с исходными объектами, поэтому
     невозможно надёжно присвоить автоинкрементные значения (init_by_db).
   - BulkInsertReplace решает эту проблему: ON CONFLICT задаёт уникальный
     индекс, по которому результаты RETURNING однозначно сопоставляются
     с объектами.
   - Для вставки без upsert используйте цикл с `Insert()`.
