...
func Test_getImportName(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "gitlab package",
			args:    args{path: "github.com/Educentr/go-activerecord/v3/internal/pkg/arerror"},
			want:    "arerror",
			wantErr: false,
		},
		{
			name:    "import with quote",
			args:    args{path: `error "github.com/Educentr/go-activerecord/v3/internal/pkg/arerror"`},
			want:    "arerror",
			wantErr: false,
		},
		{
